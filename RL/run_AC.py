...
import models.AC.model as AC_brain
import argparse
import logging
import random
import glob
...
def RL_decision(world,state,actor,window_step):
    '''
    RL decision
    ...
    :return:
    '''
    #Time window division flag
    divide_flag=0

   
    action=actor.choose_action(torch.FloatTensor(state))

    #Convert actions into time window size
...
def Simulation(args):
    ...
    try:
        ...
        actor = AC_brain.Actor(world.state_space,
                              world.action_space,
                              lr=0.001
                              )

        critic=AC_brain.Critic(world.state_space,
                               lr=0.01,
                               gamma=0.9
                               )
        ...
        for i in tqdm(range(args.simTimeSlice)):
            try:
                ...
                # change wheather
                if world.world_time % 20 == 0:
                    world.next_weather()

                state=world.get_env_state(window_step)
                divide_flag,action=RL_decision(world,state,actor,window_step)
                ...
                for worker in world.worker_list:
                    ...
                    if worker.current_task != None:
                        ...
                        print("%s sutopilot" % worker.index)
