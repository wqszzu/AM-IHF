...
class PPO(object):
    def __init__(self,n_states,n_actions,lr,gamma,clip,timesteps_per_batch,max_timesteps_per_episode,n_updates_per_iter) -> None:
        super().__init__()
        ...
        self.actor = FeedForwardNN(self.n_states, self.n_actions)
        self.critic = FeedForwardNN(self.n_states, [1])

        # create variable for the matrix and covariance matrix
        self.conv_var = t.full((len(self.n_actions),), 0.5)
        ...
        # self.sw = sw
        # self.sw.add_graph(
        #     self.critic, input_to_model=t.rand((self.n_states)))
        # self.sw.add_graph(self.actor, input_to_model=t.rand((self.n_states)))

    def learn(self, timesteps, batch_obs, batch_acts, batch_log_probs, batch_r, batch_rtgs, batch_lens):
        ...
        for i in range(self.n_updates_per_iter):
            ...
            #     timesteps, i, actor_loss.item(), critic_loss.item()))
    ...
    def get_action(self, obs):
        ...
        max_action = t.argmax(actions).data.numpy()
        return max_action,actions.detach().numpy(), log_probability.detach()
