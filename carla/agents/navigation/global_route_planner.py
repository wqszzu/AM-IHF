...
class GlobalRoutePlanner(object):
    ...
    def trace_route_length(self, origin, destination):
        """
        This method returns list of (carla.Waypoint, RoadOption)
        from origin to destination and the leangth of route
        """
        route_trace = []
        route = self._path_search(origin, destination)
        current_waypoint = self._wmap.get_waypoint(origin)
        ...
        for i in range(len(route) - 1):
            ...
            if edge['type'] != RoadOption.LANEFOLLOW and edge['type'] != RoadOption.VOID:
                ...
                next_edge = self._graph.edges[n1, n2]
                if next_edge['path']:
                    closest_index, min_distance = self._find_closest_length_in_list(current_waypoint, next_edge['path'])
                    closest_index = min(len(next_edge['path']) - 1, closest_index + 5)
                    current_waypoint = next_edge['path'][closest_index]
                else:
                    current_waypoint = next_edge['exit_waypoint']
                ...
                path = path + [edge['entry_waypoint']] + edge['path'] + [edge['exit_waypoint']]
                closest_index, min_distance = self._find_closest_length_in_list(current_waypoint, path)
                for waypoint in path[closest_index:]:
                    current_waypoint = waypoint
                    route_trace.append((current_waypoint, road_option))
                    route_length += min_distance
                    if len(route) - i <= 2 and waypoint.transform.location.distance(
                        ...
                        destination_index, min_distance = self._find_closest_length_in_list(destination_waypoint, path)
                        if closest_index > destination_index:
                            break

        return route_trace, route_length
    ...
    def _find_closest_length_in_list(self, current_waypoint, waypoint_list):
        min_distance = float('inf')
        closest_index = -1
        for i, waypoint in enumerate(waypoint_list):
            distance = waypoint.transform.location.distance(
